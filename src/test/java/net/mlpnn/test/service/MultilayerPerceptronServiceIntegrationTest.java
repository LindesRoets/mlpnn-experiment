package net.mlpnn.test.service;
...
public class MultilayerPerceptronServiceIntegrationTest {
	...
	public void testLearningStatus() throws Exception {
		...
		form.setDataSetName(DataSetInfo.IRIS.name());
		form.setLearningRate(0.5);
		form.setMomentum(0.5);
		form.setNeuronCount(11);
	...
	public void testStopLearning() throws InterruptedException {
		...
		while (runner.calculateLearningStatus().equals(LearningStatus.RUNNING)) {
			...
			Assert.assertEquals("We are expecting the learning to be in a stopped state", LearningStatus.STOPPED, runner.calculateLearningStatus());
		}
